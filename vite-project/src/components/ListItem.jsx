...
export default function ListItem({ onEdit, editId, onEditText, editText,  tasks, task , onSave, onCheck, onDelete }) {
    ...
    const handleEditSave = async (id) => {
        try {
            ...
            onSave(tasks.map(task => task.id === id ? updatedTask : task));
            onEdit(null);
            onEditText('');
        } catch (error) {
            console.error('Errore:', error);
    ...
    return (
        <li className='mb-2 flex items-center'>
            ...
            {editId === task.id ? (
                ...
                <>
                    <span className={'mr-2' + (task.completed ? ' line-through text-gray-400' : '')}>{task.texts}</span>
                    <Button onClick={() => handleDelete(task.id)}>Elimina</Button> 
                    <Button onClick={() => handleEdit(task.id, task.texts)}>Modifica attività</Button> 
                </>
            )}
        </li>
    );
